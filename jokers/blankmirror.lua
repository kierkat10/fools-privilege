...
SMODS.Joker{ --Blank Mirror
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_foolspri_blankmirror') 
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play  and not context.blueprint then
            if SMODS.get_enhancements(context.other_card)["m_glass"] == true then
                if SMODS.pseudorandom_probability(card, 'group_0_c7134c11', 1, card.ability.extra.odds, 'j_foolsprivilege_blankmirror_vampire', false) then
        ...
        if context.pseudorandom_result  and not context.blueprint then
            if (not context.result and context.identifier == "j_foolsprivilege_blankmirror_vampire") then
                return {
                    func = function()
