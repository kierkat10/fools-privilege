...
SMODS.Joker{ --Down the flush
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_foolspri_downtheflush') 
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers and context.joker_main  and not context.blueprint then
            if next(context.poker_hands["Flush"]) then
                if SMODS.pseudorandom_probability(card, 'group_0_b3d07ac6', 1, card.ability.extra.odds, 'j_foolspri_downtheflush', false) then
