...
SMODS.Joker{ --Lottery Winner
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_foolspri_lotterywinner')
        local new_numerator2, new_denominator2 = SMODS.get_probability_vars(card, 1, card.ability.extra.odds2, 'j_foolspri_lotterywinner')
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play  then
            if SMODS.get_enhancements(context.other_card)["m_lucky"] == true then
                if SMODS.pseudorandom_probability(card, 'group_0_6f215288', 1, card.ability.extra.odds, 'j_foolspri_lotterywinner', false) then
                    
                    return {repetitions = 1}
                end
                if SMODS.pseudorandom_probability(card, 'group_1_aee58414', 1, card.ability.extra.odds2, 'j_foolspri_lotterywinner', false) then
