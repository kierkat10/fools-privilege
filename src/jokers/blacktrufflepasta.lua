...
SMODS.Joker{ --Black Truffle Pasta
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_fpr_blacktrufflepasta') 
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round and context.game_over == false and context.main_eval  then
            if true then
                if SMODS.pseudorandom_probability(card, 'group_0_5d6db311', 1, card.ability.extra.odds, 'j_fpr_blacktrufflepasta', false) then
