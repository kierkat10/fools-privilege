...
SMODS.Joker { --Christmas Lights
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_fpr_christmaslights') 
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play  then
            if true then
                if SMODS.pseudorandom_probability(card, 'group_0_bcb642a3', 1, card.ability.extra.odds, 'j_fpr_christmaslights', false) then
