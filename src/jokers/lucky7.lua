...
SMODS.Joker{ --Lucky 7
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_fpr_lucky7')
        local new_numerator2, new_denominator2 = SMODS.get_probability_vars(card, 1, card.ability.extra.odds2, 'j_fpr_lucky7')
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play  then
            if (function()
                ...
                if SMODS.pseudorandom_probability(card, 'group_0_0f5d8c45', 1, card.ability.extra.odds, 'j_fpr_lucky7', false) then
                    SMODS.calculate_effect({mult = 20}, card)
                end
                if SMODS.pseudorandom_probability(card, 'group_1_98270862', 1, card.ability.extra.odds2, 'j_fpr_lucky7', false) then
