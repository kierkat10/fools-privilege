...
SMODS.Joker{ --Nutella Cookie
    ...
    loc_vars = function(self, info_queue, card)
        local numerator, denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, "j_fpr_nutellacookie")
        return {
            vars = {
    ...
    calculate = function(self, card, context)
        if context.end_of_round and context.main_eval and not context.blueprint and (G.GAME.blind.boss or G.GAME.blind.name == "Big Blind") then
            ...
            if SMODS.pseudorandom_probability(card, "j_fpr_nutellacookie", 1, card.ability.extra.odds) then
